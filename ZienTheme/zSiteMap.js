    ...
    ! function (e, o) {
        ...
        e[toc_config.jsonCallback] = function (e) {
            ...
            for (var g = 0, h = n.length; h > g; ++g) {
                l += '<div class=\"zSiteMapTable\">' + n[g] + '</div>', l += '<div class=\"zSiteMapContent\"><ol>';
                for (var _ = 0, p = i.length; p > _; ++_) {
                    ...
                    for (var w = 0, u = i[_].link.length; u > w; ++w)
                        if ("alternate" == i[_].link[w].rel) {
                            c = i[_].link[w].href;
                            break
                        } for (var v = 0, m = i[_].category.length; m > v; ++v) n[g] == i[_].category[v].term && (l += '<li><a href=\"' + c + '\" title=\"' + o.replace(/ \%new\%$/, "") + '\">' + o.replace(/ \%new\%$/, "") + '</a></li>')
                }
                l += '</ol></div>'
            }
            t.innerHTML = l, "undefined" != typeof jQuery && ($("#" + toc_config.containerId + " .zSiteMapContent").hide(), $("#" + toc_config.containerId + " .zSiteMapTable").click(function () {
                $(this).hasClass("aktif") || (toc_config.clickCallback(this), $("#" + toc_config.containerId + " .zSiteMapTable").removeClass("aktif").next().slideUp(toc_config.slideSpeed.up, toc_config.slideEasing.up, toc_config.slideCallback.up), $(this).addClass("aktif").next().slideDown(toc_config.slideSpeed.down, toc_config.slideEasing.down, toc_config.slideCallback.down))
            }).eq(toc_config.activePanel - 1).addClass("aktif").next().slideDown(toc_config.slideSpeed.down, toc_config.slideEasing.down, toc_config.slideCallback.down))
        };
        var i = o.createElement("script");
        i.src = "https://"+window.location.hostname+"/feeds/posts/summary?alt=json-in-script&max-results=9999&callback=" + toc_config.jsonCallback, "onload" == toc_config.delayLoading ? e.onload = function () {
            c.appendChild(i)
